...
06. Explicit Claude preset: `claude-opus-5-5`, high, USD 10 ceiling, persistent sessions enabled and customizations disabled. No invented availability proof.
...
09. Exclude prior verdicts and preferred conclusion under both the explicit case instruction and the current fresh-consultation rule; supply relevant facts and constraints.
...
17. Report `code:claude_timeout` without a success answer, automatic retry, budget increase or provider substitution. Retain the known prior session ID and requested settings, but do not invent a new ID or claim the interrupted turn was saved.
...
25. Preserve exact known session ID and requested override separately from unknown actual metadata. Report failed timed-out consultation. No success, invented saved turn, automatic retry, budget increase or replacement provider. Resume only on explicit request with retained or explicitly changed settings.
