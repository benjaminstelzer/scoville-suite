...
25. The requested eventual outcome is new async payment-migration behavior touching two authorization owners, not a diagnosis or hardening request. Right now, request and apply the required packaged Code references before describing the mode, risk, canonical Plan-owner discovery boundary, dirty-worktree preservation and verification requirements for a durable execution plan. Select the later verification evidence; do not create a Plan record, invoke another Skill, inspect implementation, edit project code or run the migration. Existing tests mock the provider; the worktree is dirty and publication was not requested. Classify the current operation separately from later implementation.
