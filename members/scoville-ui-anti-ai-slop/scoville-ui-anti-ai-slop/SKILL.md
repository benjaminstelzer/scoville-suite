...
**WORDPRESS SPECIALIST:** Determine ownership from the surface, regardless of
which Skill was loaded first. {{ package: standalone }}When `scoville-wordpress-ui-backend-anti-ai-slop` is active and
{{ /package }}{{ package: suite }}When `scoville-wordpress-ui-backend-anti-ai-slop` is
{{ /package }}applicable to a supported plugin-owned `wp-admin` surface, refer that surface to
