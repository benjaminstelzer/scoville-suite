...
def _check_report_structure(validation: Validation, text: str) -> None:
    """Accept the default report headings or an equivalent user-requested form."""
    if all(re.search(rf"(?m)^{re.escape(heading)}\s*$", text) for heading in REPORT_HEADINGS):
        _check_headings(validation, "REPORT.md", text, REPORT_HEADINGS)
    elif not text.strip():
        validation.error("empty_report", "REPORT.md", "Report must not be empty")
...
def validate(root: Path) -> dict[str, Any]:
    ...
    run_phase = run.get("phase") if isinstance(run, dict) else None
    final_report_check = legacy or run_phase in {"validation", "complete"}
    report_path = root / "REPORT.md"
    report = validation.read_text("REPORT.md") if final_report_check or report_path.is_file() else None
    ...
    if report is not None and final_report_check:
        _check_report_structure(validation, report)
    ...
    if legacy or run_phase in {"synthesis", "validation", "complete"}:
